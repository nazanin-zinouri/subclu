...
DECLARE PT_DT DATE DEFAULT "2023-03-01";
DECLARE PT_WINDOW_START DATE DEFAULT PT_DT - 6;
...
CREATE OR REPLACE TABLE `reddit-employee-datasets.david_bermejo.pn_test_users_for_embedding_20230412` AS (
SELECT
    PT_DT AS pt
...
WHERE DATE(v.pt) BETWEEN PT_WINDOW_START AND PT_DT
    ...
    AND v.subreddit_name IN (
        ...
        'france', 'askfrance', 'vosfinances'
        , 'themandaloriantv', 'starwarsandor', 'bookofbobafett'

        , 'india', 'mumbai', 'bangalore'
        , 'sharktankindia', 'bollywood', 'devbhoomi_uttrakhand'

        -- Recommend to r/peliculas to viewers of r/movies
        , 'movies', 'television', 'moviedetails'
        , 'peliculas', 'criticasdepeliculas', 'preguntas_de_reddit'
