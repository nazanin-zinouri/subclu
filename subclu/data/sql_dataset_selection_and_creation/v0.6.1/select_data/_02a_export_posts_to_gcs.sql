-- ==============================
-- Export POST text + metadata google cloud storage (GCS)
-- CHANGE/Update:
...
EXPORT DATA
    OPTIONS(
        uri=r'gs://${output_bucket_name}/i18n_topic_model_batch/runs/${run_id}/posts/*.parquet',
        format='PARQUET',
        overwrite=true
    ) AS
    SELECT *

    FROM `reddit-relevance.${dataset}.subclu_posts_for_modeling_${run_id}` AS t
